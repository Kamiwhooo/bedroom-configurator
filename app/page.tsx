...
export default function Home() {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-gray-950">
      ...
      {/* ── Desktop sidebar ────────────────────────────────────── */}
      <aside className="hidden md:flex flex-col w-72 shrink-0 bg-gray-900/95 border-r border-white/8 overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 z-10 bg-gray-900/95 backdrop-blur border-b border-white/8 px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-white text-sm leading-tight">Room Configurator</h1>
      ...
      <div className="flex-1 relative" ref={canvasContainerRef}>
        ...
        >
          <Suspense fallback={null}>
            ...
            <OrbitControls
              ...
              zoomSpeed={0.8}
              panSpeed={0.5}
              touches={{ ONE: 1 as unknown as THREE.TOUCH, TWO: 2 as unknown as THREE.TOUCH }}
            />
          </Suspense>
        </Canvas>
        ...
        {selectedId && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 pointer-events-none z-10">
            <div className="px-4 py-2 rounded-full bg-indigo-600/90 backdrop-blur text-white text-xs font-semibold shadow-xl border border-indigo-400/50">
              ✓ {ITEM_LABELS[selectedId as keyof typeof ITEM_LABELS]} selected
            </div>
          </div>
        )}
        ...
        <div className="md:hidden absolute bottom-6 right-4 z-20">
          <button onClick={() => setShowPanel(p => !p)}
            className="w-14 h-14 rounded-full bg-indigo-600 hover:bg-indigo-500 text-white shadow-2xl shadow-indigo-500/40 flex items-center justify-center text-2xl transition active:scale-95">
            {showPanel ? \'✕\' : \'🎨\'}
          </button>
        </div>
      </div>
      ...
      {showPanel && (
        <div
          ...
          <div
            className="bg-gray-900 rounded-t-3xl border-t border-white/10 overflow-y-auto shadow-2xl"
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-white/8">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/40">Tap objects to select them</p>
                </div>
              </div>
              <button onClick={() => setShowPanel(false)}
                className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-white/60 text-sm hover:bg-white/20">
                ✕
              </button>
            </div>
            <ColorPanel {...panelProps} />
...
// Satisfy TypeScript for THREE.TOUCH
