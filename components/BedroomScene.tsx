...
function RoomShell({ c, onSelect, sel }: { c: RoomColors; onSelect: (id: string) => void; sel: string | null }) {
  ...
  return (
    <group>
      ...
      <mesh position={[0, 0.06, -D / 2 + 0.01]} receiveShadow>
        <boxGeometry args={[W, 0.10, 0.03]} />
        <meshStandardMaterial color="#E8E2DA" roughness={0.4} />
      </mesh>
      <mesh position={[-W / 2 + 0.01, 0.06, 0]} receiveShadow>
        <boxGeometry args={[0.03, 0.10, D]} />
        <meshStandardMaterial color="#E8E2DA" roughness={0.4} />
      </mesh>
      <mesh position={[W / 2 - 0.01, 0.06, 0]} receiveShadow>
        <boxGeometry args={[0.03, 0.10, D]} />
    ...
    <group position={[-2.1 + WD / 2, 0, -0.5]}>
      ...
      {[-1.425, -0.475, 0.475, 1.425].map((z, i) => (
        <group key={i}>
          ...
          <mesh position={[WD / 2 + 0.025, WH * 0.5, z + (i % 2 === 0 ? 0.3 : -0.3)]}>
  ...
  return (
    <group position={[-0.9, 0, -2.44]}>
      ...
      <group position={[0, 0, 0.68]}>
        ...
        {([-0.20, 0.20] as number[]).flatMap(x =>
          ([-0.20, 0.20] as number[]).map(z => ({ x, z }))
        ).map(({ x, z }, i) => (
...
function Bed({ c, onSelect, sel }: { c: RoomColors; onSelect: (id: string) => void; sel: string | null }) {
  return (
    <group position={[0.65, 0, -0.85]}>
      ...
      {([[-0.78, -1.02], [0.78, -1.02], [-0.78, 1.02], [0.78, 1.02]] as [number, number][]).map(([x, z], i) => (
    ...
    <>
      {isNight ? (
        ...
        <>
          ...
          <pointLight position={[0, 2.7, 0]} intensity={0.4} color="#FFFAE8" distance={7} />
        </>
      )}
    </>
  )
