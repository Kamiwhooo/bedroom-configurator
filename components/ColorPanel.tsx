...
export default function ColorPanel({
  ...
  const inputCls = 'w-full px-3 py-2 rounded-xl border border-white/10 bg-white/5 text-white text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-400/50'
    ...
    <div className="flex flex-col gap-5 p-4 text-white">
      <div className="grid grid-cols-2 gap-2">
        <button onClick={() => setIsNight(!isNight)}
          className={`${btnSm} bg-white/10 hover:bg-white/20 col-span-1 justify-center`}>
      ...
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest text-white/40 mb-2">Select Element</p>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/40">Tap object in 3D to select</p>
          </div>
        </div>
        ...
        <div>
          <label className="block text-xs text-white/40 mb-1 font-medium">HEX</label>
        ...
        <div className="grid grid-cols-3 gap-2">
          {(['r', 'g', 'b'] as const).map(ch => (
            <div key={ch}>
              <label className="block text-xs text-white/40 mb-1 font-medium uppercase">{ch}</label>
      ...
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest text-white/40 mb-2">Quick Palettes</p>
        <div className="flex flex-col gap-2">
          {PALETTES.map((pal, pi) => (
            <div key={pi} className="flex items-center gap-2">
              <span className="text-xs text-white/40 w-16 shrink-0">{pal.name}</span>
              <div className="flex gap-1.5">
                {pal.colors.map((pc, ci) => (
                  <button key={ci} onClick={() => apply(pc)} title={pc}
                    className="w-7 h-7 rounded-lg border border-white/10 hover:scale-110 transition-all"
                    style={{ background: pc }} />
                ))}
              </div>
            </div>
